...
const MESSAGES = {
  ...
  'meta.description': { zh: '面向 macOS Apple Silicon 的本地优先 Markdown 桌面写作工具，提供可靠保存、主题系统与插件管理。', en: 'A local-first Markdown editor for Apple Silicon Macs, with reliable saving, themes, and plugin management.' },
  ...
  'home.hero.lead': { zh: '面向 macOS Apple Silicon 的 Markdown 桌面写作工具。文件留在磁盘，内容、主题和扩展能力都保持清晰可控。', en: 'A Markdown editor built for Apple Silicon Macs. Your files stay on disk, while content, themes, and extensions remain clear and controllable.' },
  ...
  'preview.body': { zh: '写作应当接近纸面：打开即写，内容留存在本地。需要时再切换源码，或导出成适合分享的格式。', en: 'Writing should feel close to paper: open and write, keep the source local, switch to Markdown when needed, and export only when the work is ready to share.' },
  ...
  'safety.lead': { zh: '从原子写入到崩溃恢复，所有写盘路径都围绕同一原则设计：不让内容在写入途中丢失。', en: 'From atomic writes to crash recovery, every write path is designed around one rule: content must not be lost mid-write.' },
  ...
  'safety.item4.title': { zh: '删除留副本', en: 'Deletion keeps a copy' },
  'safety.item4.body': { zh: '删除的文档会先移动到应用数据目录，不会被立即抹除。', en: 'Deleted documents move into the app data folder instead of being erased outright.' },
  ...
  'docs.metaDescription': { zh: '了解 Markdown Writing 的保存、删除处理和恢复机制。', en: 'How Markdown Writing handles saves, deletions, and recovery.' },
  ...
  'docs.heroLead': { zh: 'Markdown Writing 把本地文件视为唯一事实来源。保存、退出保护和恢复都围绕这条原则设计。', en: 'Markdown Writing treats local files as the source of truth. Saving, exit protection, and recovery are designed around that principle.' },
  'docs.sidebarTitle': { zh: '安全设计', en: 'Safety design' },
  'docs.nav.principles': { zh: '设计原则', en: 'Principles' },
  'docs.nav.save': { zh: '保存流程', en: 'Save flow' },
  'docs.nav.recycle': { zh: '删除与恢复', en: 'Deletion and recovery' },
  ...
  'docs.principles.body': { zh: '编辑器的第一责任不是让界面显得智能，而是在异常和误操作发生时，尽量不让用户失去已经写下的内容。', en: 'The editor prioritizes preserving written work over appearing intelligent, especially when failures or mistakes happen.' },
  ...
  'docs.principles.item3': { zh: '恢复优先于报错：异常退出和误删除都尽量提供可执行的恢复路径。', en: 'Recovery before errors: unexpected exits and accidental deletions leave an actionable recovery path where possible.' },
  ...
  'docs.save.step1.title': { zh: '写入临时文件', en: 'Write a temporary file' },
  'docs.save.step1.body': { zh: '新内容先写到目标目录中的临时文件，避免原文件在写入中被截断。', en: 'New content is written to a temporary file in the target directory so the original is never partially truncated.' },
  'docs.save.step2.title': { zh: '原子替换原文件', en: 'Replace atomically' },
  'docs.save.step2.body': { zh: '内容完整写入后替换原文件，再做必要的清理。', en: 'The original is replaced only after the new content is complete, followed by necessary cleanup.' },
  'docs.save.step3.title': { zh: '清理恢复状态', en: 'Clear recovery state' },
  'docs.save.step3.body': { zh: '保存成功后清理已经落盘的恢复内容。', en: 'After a successful save, recovery state that is no longer needed is cleared.' },
  ...
  'docs.recycle.title': { zh: '删除与恢复', en: 'Deletion and recovery' },
  'docs.recycle.body': { zh: '删除文档时文件会先移动到应用数据目录而不是立即抹除，但应用内暂不提供恢复入口；磁盘文件消失时仍可保留编辑内容并另存副本。', en: 'Deleting a document moves the file into the app data folder instead of erasing it, although the app does not offer a restore entry yet; if the disk file disappears, your content is kept and can be saved as a copy.' },
  'docs.recycle.scenario': { zh: '场景', en: 'Scenario' },
  'docs.recycle.action': { zh: '保护动作', en: 'Protection' },
  'docs.recycle.entry': { zh: '应用内入口', en: 'In-app entry' },
  'docs.recycle.row1': { zh: '误删除文档', en: 'Document deleted' },
  'docs.recycle.row1Action': { zh: '文件先移动到应用数据目录', en: 'Move the file into the app data folder' },
  'docs.recycle.row1Entry': { zh: '暂不提供', en: 'Not available yet' },
  'docs.recycle.row2': { zh: '外部文件消失', en: 'External file removed' },
  'docs.recycle.row2Action': { zh: '保留编辑内容', en: 'Keep editor content' },
  'docs.recycle.row2Entry': { zh: '另存副本', en: 'Save a copy' },
  ...
  'docs.recovery.body2': { zh: '恢复快照包含文档、内容和编辑模型版本。启动时若发现未处理快照，会询问恢复还是丢弃。', en: 'A recovery snapshot includes the document, content, and editor model version. On launch, the app asks whether to restore or discard it.' },
  'docs.boundaries.title': { zh: '能力边界', en: 'Boundaries' },
  'docs.boundaries.body': { zh: '这些机制主要防止正常使用中的误操作和进程中断，不能替代系统备份或磁盘加密。', en: 'These mechanisms reduce accidental mistakes and interrupted processes. They do not replace system backups or disk encryption.' },
  ...
  'docs.boundaries.item2': { zh: '应用数据目录保存被删除文件的副本与元数据，应纳入系统备份。', en: 'The application data directory keeps copies and metadata of deleted files and should be included in system backups.' },
